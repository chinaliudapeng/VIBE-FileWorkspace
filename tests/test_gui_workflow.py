...
def test_gui_workspace_creation_workflow():
    ...
    with tempfile.TemporaryDirectory() as temp_db_dir:
        test_db_path = Path(temp_db_dir) / 'test_workspace_indexer.db'

        # Mock the database path for testing
        with patch('core.db.get_db_path', return_value=test_db_path):
            ...
            try:
                ...
                if row_count > 0:
                    ...
                    print("  [ERROR] No files loaded into FileTableModel!")

            except Exception as e:
                print(f"  [ERROR] Error loading files into FileTableModel: {e}")
                import traceback
                traceback.print_exc()
            ...
            if files_added > 0 and len(db_files) > 0:
                if file_table_model.rowCount() > 0:
                    print("[OK] GUI workflow successful: Files created, scanned, and loaded into GUI model")
                else:
                    print("[ERROR] Issue found: Files scanned but not loaded into GUI model")
            else:
                print("[ERROR] Issue found: Files not properly scanned into database")
